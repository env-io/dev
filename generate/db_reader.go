...
func (tag *OrmTag) String() string {
	...
	if tag.RelFk {
		...
		if strings.HasSuffix(tag.Column, "_id") {
			tag.Column = core.RightTrim(tag.Column, "_id")
			omitempty = ",omitempty"
		}
	}
	if tag.RelOne {
...
func (*MysqlDB) GetTableNames(db *sql.DB) (tables []string) {
	...
	if err != nil {
		core.LogError("Could not show tables, Please check your connection string.")
		os.Exit(2)
	}
	defer rows.Close()
	for rows.Next() {
		...
		if err := rows.Scan(&name); err != nil {
			core.LogError("Could not show tables.")
			os.Exit(2)
		}
		if name != "migrations" {
...
func (*MysqlDB) GetConstraints(db *sql.DB, table *Table, blackList map[string]bool) {
	...
	if err != nil {
		core.LogError("Could not get query INFORMATION_SCHEMA for PK/UK/FK information.")
		os.Exit(2)
	}
	for rows.Next() {
...
func (*MysqlDB) GetGoDataType(dataType string) (goType string) {
	if v, ok := mysqlDataType[dataType]; ok {
		...
		core.LogError(fmt.Sprintf("data type (%s) not found.", dataType))
		os.Exit(2)
	}
	return goType
}
...
func (m *MysqlDB) GetColumns(db *sql.DB, table *Table, blackList map[string]bool) {
	...
	for cols.Next() {
		...
		if err := cols.Scan(&colNameBytes, &dataTypeBytes, &columnTypeBytes, &isNullableBytes, &columnDefaultBytes, &extraBytes); err != nil {
			core.LogError("Could not get query INFORMATION_SCHEMA for column information.")
			os.Exit(2)
		}
		...
		// create a column
		col := new(Column)
		col.Name = core.ToCamelCase(colName)
		...
		if table.Pk == colName {
			...
			if isFk && !isBl {
				...
				if strings.HasSuffix(colName, "_id") {
					col.Name = core.RightTrim(col.Name, "Id")
				}

				col.Type = "*" + core.ToCamelCase(refStructName)
